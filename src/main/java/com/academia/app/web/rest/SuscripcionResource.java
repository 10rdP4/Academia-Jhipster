...
public class SuscripcionResource {
    ...
    /**
     * {@code GET  /suscripcions/taller/:id_alumno} : get all the suscripcions by id_taller.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of suscripcions in body.
     */
    @GetMapping("/suscripcions/alumno/{idalumno}")
    public ResponseEntity<List<Suscripcion>> getAllSuscripcionsByIdAlumno(@org.springdoc.api.annotations.ParameterObject Pageable pageable, @PathVariable Integer idalumno) {
        log.debug("REST request to get a page of Suscripcions by Id Alumno");
        Page<Suscripcion> page = suscripcionService.findAllByIdAlumno(pageable, idalumno);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
